...
Ejecuta automáticamente la cadena diaria de extracción (PRMs → manifiestos → combustibles) en el horario configurado y recupera las corridas que se hayan perdido.

## ADDED Requirements

### Requirement: Corrida diaria programada

El sistema SHALL ejecutar automáticamente la cadena de extracción todos los días a la hora configurada (por defecto 15:00 en la zona `America/La_Paz`), en el orden PRMs, luego manifiestos y por último combustibles.
...
- **THEN** primero corren los PRMs, después los manifiestos y por último los combustibles
