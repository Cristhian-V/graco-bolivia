...
- **THEN** `dim_dam` es el código `DI` si hay DIM y si no el código de la DAM; `us_unitario` es el precio unitario multiplicado por 1000; `tramo_flete` combina el lugar de embarque y el departamento de destino; `flete_total_usd` y `flete_total_bs` se derivan del total de la factura de transporte y su moneda, convirtiendo con el tipo de cambio; `tarifa_flete_usd_m3` es `flete_total_usd` dividido entre `cantidad_m3`
